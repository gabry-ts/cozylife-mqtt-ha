...
class CozylifeMQTTBridge:
    def __init__(self, device_ip="192.168.1.100", device_port=5555,
                 mqtt_broker="192.168.1.1", mqtt_port=1883,
                 mqtt_user=None, mqtt_password=None,
                 device_name="cozylife_light"):
        ...
        self.device_display_name = device_name.replace("_", " ").title()
    ...
    def send_discovery_config(self):
        ...
        config = {
            ...
            "unique_id": f"cozylife_{self.device_id}",
    ...
    def on_mqtt_message(self, client, userdata, msg):
        ...
        if topic == self.command_topic:
            ...
            try:
                ...
                brightness_ha = max(0, min(255, brightness_ha))

                # Original conversion: payload['4'] = brightness * 4
            ...
            try:
                mireds = int(payload)
                # Original conversion: payload['3'] = 1000 - colortemp * 2
    ...
    def query_and_publish_state(self):
        ...
        if response and "msg" in response and "data" in response["msg"]:
            ...
            # Get brightness (attribute 4: scale 0-4000 in original integration)
            brightness_cozy = data.get("4", 1000)
            # Original conversion: brightness = int(state['4'] / 4)
            brightness_ha = int(brightness_cozy / 4)
    ...
    def start(self):
        ...
        print(f"[Bridge] Entity ID in HA: light.{self.device_id}")
...
if __name__ == "__main__":
    ...
    device_name = os.getenv('DEVICE_NAME', 'cozylife_light')

    print(f"[Config] Device: {device_ip}:{device_port}")
    print(f"[Config] MQTT: {mqtt_broker}:{mqtt_port}")
    print(f"[Config] Device name: {device_name}")
